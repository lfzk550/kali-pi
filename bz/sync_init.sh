...
if [ ! -d "/mnt/workspace" ] && [ -z "${S3_BUCKET}" ] && [ -z "${WEBDAV_URL}" ]; then
    echo "/mnt/workspace 当前为非 ModelScope 环境，且未配置 S3/WEBDAV 远程储存，不执行 Pi 配置自动恢复任务"
    exit 1
fi

echo "正在恢复容器中 Pi 的历史配置，请稍后..."
...
if [ -n "${S3_BUCKET}" ]; then
    ...
    rclone copyto ":s3:${S3_BUCKET}/${S3_BACKUP_PATH:-backups/data_pi.tar.gz}" /tmp/data.tar.gz \
        --s3-provider Other \
    ...
    rclone copyto ":webdav:/${WEBDAV_BACKUP_PATH:-backups/data_pi.tar.gz}" /tmp/data.tar.gz \
        --webdav-vendor other \
